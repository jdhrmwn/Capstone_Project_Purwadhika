    ...
    # Menu 4 hapus koleksi
        ...
        while True:
            ...
            # Checkout
                ...
                # tampilkan cart dulu
                Data_Cart = []
                for idb in Cart_Peminjaman:
                    idx = cari_index_buku_by_id(DB_Perpus, idb)
                    if idx != -1:
                        Data_Cart.append(DB_Perpus[idx])
                ...
                for idb in Cart_Peminjaman:
                    idx = cari_index_buku_by_id(DB_Perpus, idb)
                    if idx == -1:
                        continue  # harusnya gak kejadian kalau cart valid

                    # safety check stok (kalau ada perubahan stok di luar cart)
                    if DB_Perpus[idx][5] <= 0:
